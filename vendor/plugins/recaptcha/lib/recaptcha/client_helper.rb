  ...
  module ClientHelper
    ...
    def recaptcha_tags(options = {})
      ...
      if options[:ajax]
        ...
        html << %{</script>\n}
        html << %{<script type="text/javascript">\n}
        html << %{ var RecaptchaOptions = {theme :'clean',lang : 'pt'} }
        html << %{</script>\n}
      else
        html << %{<script type="text/javascript" src="http://api.recaptcha.net/js/recaptcha.js">var RecaptchaOptions = {theme :'clean',lang : 'pt'}</script>\n}
        html << %{<script type="text/javascript" src="#{uri}/challenge?k=#{key}}
        html << %{#{error ? "&error=#{CGI::escape(error)}" : ""}"> var RecaptchaOptions = {theme : 'clean',lang : 'pt'}</script>\n}
